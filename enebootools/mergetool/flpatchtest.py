...
def diff_test(iface, base, final):
    iface.debug(u"Procesando Diff PY_test $base:%s -> $final:%s" % (base, final))
    nbase, flbase = file_reader(base)
    nfinal, flfinal = file_reader(final)
    if flbase is None or flfinal is None:
        iface.info(u"Abortando Diff PY_test por error al abrir los ficheros")
        return
...
def extract_classes_test(iface, final, classlist):
    ...
    iface.debug(u"Extrayendo clases PY_test $final:%s $classlist:%s" % (final, ",".join(classlist)))
    nfinal, flfinal = file_reader(final)
    if flfinal is None:
...
def split_test_old(iface, final):
    iface.debug(u"Separando fichero PY_test %s . . . " % (final))
    nfinal, flfinal = file_reader(final)
    if flfinal is None:
        iface.info(u"Abortando por error al abrir los ficheros")
        return
...
def patch_test_dir(iface, base, patch):
    iface.debug(u"Procesando Patch sobre carpeta test $base:%s + $patch:%s" % (base, patch))
    base_filename = base
...
def diff_test_dir(iface, base, final):
    iface.debug(u"Procesando Diff de carpetas PY_test $base:%s -> $final:%s" % (base, final))
...
def check_test_classes(iface, base):
    iface.debug(u"Comprobando clases del fichero PY_test $filename:%s" % (base))
    nbase, flbase = file_reader(base)
    if flbase is None:
...
def patch_test(iface, base, patch):
    iface.debug(u"Procesando Patch PY_test $base:%s + $patch:%s" % (base, patch))
    nbase, flbase = file_reader(base)
    npatch, flpatch = file_reader(patch)
    if flbase is None or flpatch is None:
        iface.info(u"Abortando Patch PY_test por error al abrir los ficheros")
        return
    # classlist
