...
def diff_py(iface, base, final):
    iface.debug(u"Procesando Diff PY $base:%s -> $final:%s" % (base, final))
    nbase, flbase = file_reader(base)
    nfinal, flfinal = file_reader(final)
    if flbase is None or flfinal is None:
        iface.info(u"Abortando Diff PY por error al abrir los ficheros")
        return
...
def extract_classes_py(iface, final, classlist):
    ...
    iface.debug(u"Extrayendo clases PY $final:%s $classlist:%s" % (final, ",".join(classlist)))
    nfinal, flfinal = file_reader(final)
    if flfinal is None:
...
def split_py_old(iface, final):
    iface.debug(u"Separando fichero PY %s . . . " % (final))
    nfinal, flfinal = file_reader(final)
    if flfinal is None:
        iface.info(u"Abortando por error al abrir los ficheros")
        return
...
def patch_py_dir(iface, base, patch):
    iface.debug(u"Procesando Patch sobre carpeta py $base:%s + $patch:%s" % (base, patch))
    base_filename = base
...
def diff_py_dir(iface, base, final):
    iface.debug(u"Procesando Diff de carpetas PY $base:%s -> $final:%s" % (base, final))
...
def check_py_classes(iface, base):
    iface.debug(u"Comprobando clases del fichero PY $filename:%s" % (base))
    nbase, flbase = file_reader(base)
    if flbase is None:
...
def patch_py(iface, base, patch):
    iface.debug(u"Procesando Patch PY $base:%s + $patch:%s" % (base, patch))
    nbase, flbase = file_reader(base)
    npatch, flpatch = file_reader(patch)
    if flbase is None or flpatch is None:
        iface.info(u"Abortando Patch PY por error al abrir los ficheros")
        return
    # classlist
