...
def diff_model(iface, base, final):
    iface.debug(u"Procesando Diff PY_model $base:%s -> $final:%s" % (base, final))
    nbase, flbase = file_reader(base)
    nfinal, flfinal = file_reader(final)
    if flbase is None or flfinal is None:
        iface.info(u"Abortando Diff PY_model por error al abrir los ficheros")
        return
...
def extract_classes_model(iface, final, classlist):
    ...
    iface.debug(u"Extrayendo clases PY_model $final:%s $classlist:%s" % (final, ",".join(classlist)))
    nfinal, flfinal = file_reader(final)
    if flfinal is None:
...
def split_model_old(iface, final):
    iface.debug(u"Separando fichero PY_model %s . . . " % (final))
    nfinal, flfinal = file_reader(final)
    if flfinal is None:
        iface.info(u"Abortando por error al abrir los ficheros")
        return
...
def patch_model_dir(iface, base, patch):
    iface.debug(u"Procesando Patch sobre carpeta model $base:%s + $patch:%s" % (base, patch))
    base_filename = base
...
def diff_model_dir(iface, base, final):
    iface.debug(u"Procesando Diff de carpetas PY_model $base:%s -> $final:%s" % (base, final))
...
def check_model_classes(iface, base):
    iface.debug(u"Comprobando clases del fichero PY_model $filename:%s" % (base))
    nbase, flbase = file_reader(base)
    if flbase is None:
...
def patch_model(iface, base, patch):
    iface.debug(u"Procesando Patch PY_model $base:%s + $patch:%s" % (base, patch))
    nbase, flbase = file_reader(base)
    npatch, flpatch = file_reader(patch)
    if flbase is None or flpatch is None:
        iface.info(u"Abortando Patch PY_model por error al abrir los ficheros")
        return
    # classlist
    ...
    for newclass in clpatch['classes'] + clpatch['delclasses']:
        ...
        if str(newclass).lower() in lower_classes:
            ...
            if iface.patch_model_rewrite == "no":
                iface.warn(
                    u"La clase %s ya estaba insertada en el fichero, "
        ...
        if extends is None:
            iface.error(
                u"La clase %s carece de extends y no es insertable como"
        ...
        if extends not in clbase['classes']:
            ...
            if clsheur == 1:
                try:
                    ...
                    iface.warn(
                        u"La clase %s debía heredar de %s, pero no "
                        u"la encontramos en el fichero base. "
                        u"En su lugar, heredará de %s." % (newclass, extends, modelclass)
                    )
                    extends = modelclass
                except IndexError:
                    iface.error(
        ...
        if mode == "insert":
            ...
            if cdpatch[newclass]['extends'] != extending:
                iface.debug(
                    u"La clase %s extendía %s en el parche, pasará a"
                    u" heredar a la clase %s" % (
                        newclass,
